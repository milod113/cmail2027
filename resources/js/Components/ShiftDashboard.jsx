...
/**
 * @param {{ tasks?: Array<any>, mode?: 'active' | 'archived' }} props
 */
export default function ShiftDashboard({ tasks = [], mode = 'active' }) {
    ...
    const destroyTask = (taskId) => {
        if (!window.confirm(__('Supprimer definitivement cette tache ?'))) {
            return;
        }
            ...
            <div className="border-b border-cyan-100/80 bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.18),_transparent_42%),linear-gradient(135deg,_rgba(240,249,255,0.96),_rgba(255,255,255,0.9))] px-6 py-6 dark:border-slate-800 dark:bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.14),_transparent_42%),linear-gradient(135deg,_rgba(15,23,42,0.96),_rgba(2,6,23,0.92))]">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex items-start gap-4">
                        <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-gradient-to-br from-cyan-500 via-sky-500 to-teal-500 text-white">
                        ...
                        <div>
                            ...
                            <h2 className="mt-3 text-2xl font-bold text-slate-900 dark:text-white">
                                {isArchivedMode ? __('Archives des taches') : __('Taches issues des messages')}
                            </h2>
                            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                {isArchivedMode
                                    ? __('Consultez, restaurez ou supprimez les taches archivees.')
                    ...
                    {route().has('tasks.archives') && route().has('tasks.index') ? (
                        ...
                        >
                            ...
                            {isArchivedMode ? __('Voir les taches actives') : __('Voir les archives')}
                        </Link>
                    ) : null}
                </div>
            </div>
                ...
                {tasks.length > 0 ? (
                    <div className="space-y-4">
                        {tasks.map((task) => {
                            ...
                            const dueDateLabel = formatDate(task.due_date, locale);
                                    ...
                                    <div className="flex items-start gap-4">
                                        {!isArchivedMode ? (
                                            <button
                                                ...
                                                aria-label={isCompleted ? __('Marquer comme en attente') : __('Marquer comme terminee')}
                                            >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                                <div>
                                                    ...
                                                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                                        {task.description || __('Aucune description fournie pour cette tache.')}
                                                    </p>
                                                </div>
                                                ...
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${
                                                        ...
                                                        {isCompleted ? __('Terminee') : __('En attente')}
                                                    </span>
                                                    {task.message_id ? (
                                            ...
                                            <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                                                {createdAtLabel ? <span>{__('Creee le')} {createdAtLabel}</span> : null}
                    ...
                    <div className="rounded-3xl border border-dashed border-cyan-200 bg-cyan-50/50 px-6 py-12 text-center dark:border-cyan-500/20 dark:bg-cyan-500/5">
                        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-3xl bg-white text-cyan-600 dark:bg-slate-900 dark:text-cyan-300">
                            <ClipboardList className="h-6 w-6" />
                        </div>
                        <h3 className="mt-4 text-lg font-semibold text-slate-900 dark:text-white">
                            {isArchivedMode ? __('Aucune archive disponible') : __('Aucune tache pour ce service')}
                        </h3>
                        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                            {isArchivedMode
                                ? __('Archivez une tache pour la retrouver ici plus tard.')
                                : __('Creez une tache depuis un message important pour suivre les actions a realiser.')}
                        </p>
                    </div>
                )}
            </div>
